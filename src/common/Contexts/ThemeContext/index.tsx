import { ReactNode, createContext, memo, useCallback, useMemo, useState } from "react";
...
function getThemeFromLocalStorage(): Theme {
  const themeTypeGuard = (theme: string): theme is Theme => ['light', 'dark'].includes(theme as Theme)
  const userTheme = localStorage.getItem(LOCAL_STORAGE_THEME_KEY)
  return themeTypeGuard(userTheme) ? userTheme : 'light'
}

export const ThemeContextProvider = memo(function ThemeContextProvider(props: { children: ReactNode }) {
